...
public class SirenOrderHomeViewHandler {
    ...
    public void whenDeliveryStarted_then_UPDATE_2(@Payload DeliveryStarted deliveryStarted) {
        try {
            if (deliveryStarted.isMe()) {
                // view 객체 조회
            	List<SirenOrderHome> sirenOrderHomeList = sirenOrderHomeRepository.findByOrderId(deliveryStarted.getOrderId());
                for(SirenOrderHome sirenOrderHome : sirenOrderHomeList){
                	sirenOrderHome.setShopId(deliveryStarted.getId());
                	sirenOrderHome.setStatus("DeliveryStarted");
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
                    // view 레파지 토리에 save
                	sirenOrderHomeRepository.save(sirenOrderHome);
                }
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenDeliveryCancelled_then_DELETE_3(@Payload DeliveryCancelled deliveryCancelled) {
        try {
            if (deliveryCancelled.isMe()) {
                // view 객체 조회
            	List<SirenOrderHome> sirenOrderHomeList = sirenOrderHomeRepository.findByOrderId(deliveryCancelled.getOrderId());
                for(SirenOrderHome sirenOrderHome : sirenOrderHomeList){
                	sirenOrderHome.setShopId(deliveryCancelled.getId());
                	sirenOrderHome.setStatus("DeliveryCancelled");
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
                    // view 레파지 토리에 save
                	sirenOrderHomeRepository.save(sirenOrderHome);
                }
        ...
        try {
            if (orderCancelled.isMe()) {
            	...
            	for(SirenOrderHome sirenOrderHome : sirenOrderHomeList){
                	...
                	sirenOrderHome.setStatus("OrderCancelled");
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
                    // view 레파지 토리에 save
                	sirenOrderHomeRepository.save(sirenOrderHome);
                }
